...
// Installed from npm, `npx -y agentline` is right. Run out of a git checkout —
...
function launcherFor(packageName = "agentline") {
  const entry = require.resolve("../../bin/agentline.js");
  if (entry.includes(`${path.sep}node_modules${path.sep}`)) return `npx -y ${packageName}`;
  return `node ${entry}`;
}

function commandFor(host, format, packageName = "agentline", launcher = launcherFor(packageName)) {
